...
```bash
docker build --build-arg JRE_TAG=8-jre-slim -t us.gcr.io/echoparklabs/proj.4:8-jre-slim .
```
To build the latest debian 10 jdk:
```bash
docker build --build-arg JDK_TAG=10-jdk-slim -t us.gcr.io/echoparklabs/proj.4:10-jdk-slim .
```
